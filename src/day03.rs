...
fn get_map() -> Vec<Vec<bool>> {
    get_input()
        .map(|line| line.chars().map(|square| square == '#').collect::<Vec<bool>>())
        .collect()
}
...
fn count_trees(forest: &Vec<Vec<bool>>, slope: &Slope) -> u8 {
    ...
    loop {
        ...
        let (new_row, new_column) =
            slope.move_toboggan(row_index, column_index, row.len());
        row_index = new_row;
...
impl Slope {
    ...
    fn move_toboggan(&self, current_row: usize,
                     current_column: usize,
                     num_columns: usize) -> (usize, usize) {
