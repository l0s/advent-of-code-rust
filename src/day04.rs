...
mod tests {
    ...
    fn part2() {
        ...
        let count_valid = get_blocks()
            .filter(|block| -> bool {
                let birth_year: Option<u16> = block
                    .get("byr")
            ...
            .filter(|block| -> bool {
                let issue_year: Option<u16> = block
                    .get("iyr")
            ...
            .filter(|block| -> bool {
                let expiration_year: Option<u16> = block
                    .get("eyr")
            ...
            .filter(|block| -> bool {
                let height = block.get("hgt").filter(|string| -> bool {
            ...
            .filter(|block| -> bool {
                let hair_colour = block
                    .get("hcl")
                    .filter(|colour| valid_hair_colour.is_match(colour));
                hair_colour.is_some()
            })
            .filter(|block| -> bool {
                let eye_colour = block
                    .get("ecl")
                    .filter(|colour| valid_eye_colours.contains(colour as &str));
                eye_colour.is_some()
            })
            .filter(|block| -> bool {
                let passport_id = block.get("pid").filter(|id| valid_passport_id.is_match(id));
                passport_id.is_some()
            })
            .count();
