...
fn get_input() -> impl Iterator<Item=Vec<String>> {
    get_block_strings("/input/day-6-input.txt")
        .map(|string| string.split_whitespace()
...
mod tests {
    ...
    fn part1() {
        let sum: u16 = get_input()
            .map(|group| -> u16 { // number of questions to which group answered yes
                group.iter()
    ...
    fn part2() {
        let sum: u16 = get_input()
            .map(|group| -> u16 {
                ...
                let unique_characters = group.iter()
                    .flat_map(|string| string.chars())
                    .collect::<HashSet<char>>();
                unique_characters.iter().filter(|c| -> bool { // whether or not c was answered by all in group
                    group.iter()
